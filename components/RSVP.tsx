  ...
  const handleSubmit = async (event: SubmitEvent) => {
    ...
    try {
      ...
      const result = await supabase
        ...
        .eq("unique_code", code);
      const data = result.data as Guest[] | null;
      const queryError = result.error;

      if (queryError) {
        setError(queryError.message || "Unable to load RSVP details.");
        setGuests(null);
      } else if (!data || data.length === 0) {
        setError("No guests found for that code. Please check your code and try again.");
        setGuests([]);
      } else {
        setGuests(data);
      }
